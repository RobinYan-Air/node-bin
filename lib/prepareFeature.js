...
const setEnvVarible = async () => {
  try {
    const path = await searchPath('dev.js')
    ...
    shelljs.echo(`__AWX_DEPLOYMENT_PATH__="${path.substr(0, path.length - 1)}"`).toEnd('~/.bash_profile')
    shelljs.shelljs('source ~/.bash_profile')
    spinner.stop()
  } catch(e) {
    spinner.stop()
  }
